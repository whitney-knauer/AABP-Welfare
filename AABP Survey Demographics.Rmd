...
cha <- read_csv("C:/Users/knaue020/Box/RESEARCH/PAIR HOUSING/AABP WELFARE/SURVEY/DATA/CHA 10312023.csv") 
...
#Some Data Play with R using ChatGPT (hooray for AI)
...
#Look at this same data by gender (maybe)
...
**This is by Gender 1** Non binary and prefer not to say were included in the same group.
...
#Look at this same data by age. Dichotomized age to be "young" = 25-35, 36-45; "old"=over 46
...
#Look at this same data by country
...
#now same thing but with Top dairy in US
...
##Same thing but with Dairy time (proportion of time working with dairy farms)
...
*Put IN SAME MODEL?* For the rank of preference for ideal way to house dairy calves.
...
cha_mf$Country <- relevel(cha_mf$Country, ref = "US")
...
**Same model** But with Age not dichotomized. 
```{r}
#can we put all these in the same model? 
...
#Now on to acceptability of differnt practices
...
#Now to look at the same responses by other variables. First Gender
...
#Acceptability of housing managmetn practices by grad year
...
#Acceptability of different housing practices by country
...
*Put IN SAME MODEL?* For the rank of acceptability of different housing practices.
...
#General Public Most and Least Acceptable
...
#feasibility of the transition from individual to social housing? #how many participants had clients that socially housed? #How many participants had clients that reverted?\
#What most common in 20years
...
\*\*Feasibility parsed\* By differnet predictors. And Modeled.
...
*Most Common by other predictors* And ordinal Lregression model
...
model <- polr(MostComm20y~ Gender1 + Age1 + Country , data = cha_mf, method="probit", Hess=TRUE)
summary(model)
logLik(model)
pR2(model)
